...
export const getCompilerOptions = (initialFilePath: string) => {
    const { tsConfigFilePath, tsConfigPath } = getTsConfigFilePath(initialFilePath);
    if (!tsConfigFilePath) {
        return undefined;
    }

    const configFileContent = fs.readFileSync(tsConfigFilePath, "utf-8");
    const parsedConfig = ts.parseConfigFileTextToJson(tsConfigFilePath, configFileContent);
    ...
    const config = ts.parseJsonConfigFileContent(parsedConfig.config, ts.sys, path.dirname(tsConfigFilePath));
    return {
        tsConfigFilePath,
        tsConfigPath,
